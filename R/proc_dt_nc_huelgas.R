...
llave = merge(llave, read_xlsx("input/data/dt/CAE_DT_armonizado.xlsx") %>% 
                select(codigoactividad, ID2) %>% unique,
...
hoja1 = read_xlsx("input/data/dt/1590_NC_y_Huelgas_2005_2023.xlsx",
  ...
  mutate(cae_dt = as.numeric(ifelse(nchar(cae_dt) == 5, 
                             str_extract(cae_dt, "\\d{1}"), 
                             str_extract(cae_dt, "\\d{2}"))),
         ...
         #                    cae_dt %in% c(99) ~ 43,
         #                    TRUE ~ NA_integer_),
         ID2 = factor(case_when(!is.na(cae_dt) ~ cae_dt,
                              !is.na(ID2) ~ ID2,
                              TRUE ~ NA_integer_)),
...
# hoja1 %>% 
#   mutate(info = case_when(is.na(codigoactividad) & !is.na(cae_dt) ~ "Sólo DT",
#                           !is.na(codigoactividad) & is.na(cae_dt) ~ "Sólo SII",
#                           !is.na(codigoactividad) & !is.na(cae_dt) ~ "Ambas",
#                           TRUE ~ "Ninguna")) %>% 
#   group_by(info) %>%
#   count %>% 
