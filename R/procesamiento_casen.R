...
c98 = read_dta("input/data/casen1998.dta")
c00 = read_dta("input/data/casen2000.dta")
c03 = read_dta("input/data/casen2003.dta")
c06 = read_dta("input/data/casen2006.dta")
c09 = read_dta("input/data/casen2009.dta")
...
c98 = c98 %>% 
  select(segmento, #paste0 para id hogar
         f, 
         exp = expr,
         strat = estrato,
         company_size = o10, #5 y 6
         contract_duration = o13, #1 = permanente
         rama,
         cise = o8) %>% 
  mutate(across(everything(), ~as.numeric(.)),
         id = paste0(segmento, "-", f),
         salaried_selfempl = ifelse(cise %in% c(2,3,4,5), "1", "0"), #No identifica privados
         company_size = ifelse(company_size %in% c(5,6), "1", "0"),
         contract_duration = ifelse(contract_duration == 1, "1", "0"),
...
c00 = c00 %>% 
  select(segmento,
         ...
         exp = expr,
         strat = estrato,
         contract_duration = o15,
         company_size = o13,
         rama,
         cise = o10) %>%
  mutate_at(vars(everything(), -company_size), ~as.numeric(.)) %>% 
  mutate(id = paste0(segmento, "-", folio),
         contract_duration = ifelse(contract_duration == 1, "1", "0"),
...
c03 = c03 %>% 
   select(segmento,
         ...
         exp = expr,
         strat = estrato,
         contract_duration = o10,
         company_size = o14,
         rama, 
         cise = o9) %>% 
  mutate_at(vars(everything(), -company_size), ~as.numeric(.)) %>% 
  mutate(id = paste0(segmento, "-", f),
         contract_duration = ifelse(contract_duration == 1, "1", "0"),
         company_size = ifelse(company_size %in% c("E", "F"), "1", "0"),
         rama = factor(case_when(rama == 1 ~ "1. Agricultura, ganadería, silvicultura y pesca",
                                 rama == 2 ~ "2. Minería",
...
c06 = c06 %>% 
  ...
  mutate(id = paste0(segmento, "-", f),
         contract_duration = ifelse(contract_duration == 1, "1", "0"),
         company_size = ifelse(company_size %in% c("E", "F"), "1", "0"),
         job_seniority = ifelse(!is.na(job_seniority) & job_seniority != 9999, 2006-job_seniority, NA),
         rama = factor(case_when(rama == 1 ~ "1. Agricultura, ganadería, silvicultura y pesca",
                                 rama == 2 ~ "2. Minería",
                                 rama == 3 ~ "3. Industrias manufactureras",
                                 rama == 4 ~ "4. Suministro de electricidad, gas y agua",
...
c09 = c09 %>% 
  ...
  mutate(contract_duration = ifelse(contract_duration == 1, "1", "0"),
         company_size = ifelse(company_size %in% c("E", "F"), "1", "0"),
         job_seniority = ifelse(!is.na(job_seniority) & job_seniority != 9999, 2009-job_seniority, NA),
         rama = factor(case_when(rama == 1 ~ "1. Agricultura, ganadería, silvicultura y pesca",
                                 rama == 2 ~ "2. Minería",
                                 rama == 3 ~ "3. Industrias manufactureras",
                                 rama == 4 ~ "4. Suministro de electricidad, gas y agua",
...
a = list(c98, c00, c03, c06, c09)

saveRDS(a, "output/data/casen_proc.rds")
